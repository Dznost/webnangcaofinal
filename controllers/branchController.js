  ...
  try {
    ...
    const enriched = await Promise.all(
      branches.map(async (branch) => {
        ...
        // Get all employees assigned to this branch with their roles
        const [receptionists, staffMembers, shippers] = await Promise.all([
          User.find({ role: "reception", branchId: branch._id })
            .select("name email phone role")
            .lean(),
          User.find({ role: "staff", branchId: branch._id })
            .select("name email phone role")
            .lean(),
          User.find({ role: "shipper", branchId: branch._id })
            .select("name email phone role")
            .lean(),
        ])

        b.receptionists = receptionists
        b.staffMembers = staffMembers
        b.shippers = shippers
        b.totalEmployees = receptionists.length + staffMembers.length + shippers.length

        // Active orders (dine-in + takeaway from this branch)
        const [activeOrdersDineIn, activeOrdersTakeaway, completedOrdersDineIn, completedOrdersTakeaway] = await Promise.all([
          Order.countDocuments({ branchId: branch._id, orderType: "dine-in", status: "processing" }),
          Order.countDocuments({ branchId: branch._id, orderType: { $ne: "dine-in" }, status: { $in: ["processing", "shipping"] } }),
          Order.countDocuments({ branchId: branch._id, orderType: "dine-in", status: "completed" }),
          Order.countDocuments({ branchId: branch._id, orderType: { $ne: "dine-in" }, status: "completed" }),
        ])

        b.activeOrders = activeOrdersDineIn
        b.activeDeliveryOrders = activeOrdersTakeaway
        b.completedOrders = completedOrdersDineIn + completedOrdersTakeaway

        // Total branch revenue (both reception + delivery payments linked to this branch)
        const revenueResult = await Payment.aggregate([
          { $match: { branchId: branch._id, status: "completed" } },
          { $group: { _id: null, total: { $sum: "$finalAmount" } } },
        ])
        b.branchRevenue = revenueResult[0]?.total || 0

        // Breakdown by type
        const revenueByType = await Payment.aggregate([
          { $match: { branchId: branch._id, status: "completed" } },
          { $group: { _id: "$revenueType", total: { $sum: "$finalAmount" }, count: { $sum: 1 } } },
        ])
        b.receptionRevenue = 0
        b.deliveryRevenue = 0
        revenueByType.forEach(function(r) {
          if (r._id === "reception") b.receptionRevenue = r.total
          else if (r._id === "delivery") b.deliveryRevenue = r.total
        })

        // Monthly revenue for the current year (for chart)
        var currentYear = new Date().getFullYear()
        var yearStart = new Date(currentYear, 0, 1)
        var yearEnd = new Date(currentYear, 11, 31, 23, 59, 59)
        const monthlyData = await Payment.aggregate([
          { $match: { branchId: branch._id, status: "completed", createdAt: { $gte: yearStart, $lte: yearEnd } } },
          { $group: { _id: { month: { $month: "$createdAt" }, type: "$revenueType" }, total: { $sum: "$finalAmount" } } },
        ])
        b.monthlyReception = Array(12).fill(0)
        b.monthlyDelivery = Array(12).fill(0)
        monthlyData.forEach(function(m) {
          var idx = m._id.month - 1
          if (m._id.type === "reception") b.monthlyReception[idx] = m.total
          else if (m._id.type === "delivery") b.monthlyDelivery[idx] = m.total
        })
    ...
    console.error("[restaurant] Error in getBranches:", error)
    res.redirect("/admin")
  }
}
...
exports.viewDetail = async (req, res) => {
  try {
    ...
    const events = await Event.find({
      $or: [{ discountScope: "global" }, { branches: branch._id }],
