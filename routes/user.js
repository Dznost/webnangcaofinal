  ...
  try {
    ...
    if (dishes) {
      for (const [dishId, dishData] of Object.entries(dishes)) {
        const quantity = parseInt(dishData.quantity) || 0
        if (quantity > 0) {
...
router.post("/order", checkAuth, async (req, res) => {
  try {
    const { deliveryAddress, fullName, email, phone, orderType, branchId, guests, specialRequests, paymentTiming } = req.body
    const cart = req.session.cart || []
    ...
    if (paymentTiming === 'cod') {
      // COD: redirect to profile, admin will mark as paid when delivered
      res.redirect("/user/profile?success=Đặt hàng thành công! Thanh toán khi nhận hàng")
    } else {
      // Prepaid: redirect to payment page
